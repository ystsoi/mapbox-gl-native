...
static LatLng latLngFromMercator(Point<double> mercatorCoordinate,
                                 LatLng::WrapMode wrapMode = LatLng::WrapMode::Unwrapped) {
...
void TransformState::updateCameraState() const {
    ...
    // position of the map, not the camera. Moving map 10 units left has same effect as moving camera 10 units to the
    // right.
...
FreeCameraOptions TransformState::getFreeCameraOptions() const {
    ...
    options.mercatorPosition = camera.getPosition();
    options.focusPoint = getLatLng();

    return options;
}

void TransformState::setFreeCameraOptions(const FreeCameraOptions& options) {
    if (!options.mercatorPosition && !options.focusPoint) return;

    if (options.mercatorPosition && options.mercatorPosition.value() != camera.getPosition()) {
        vec3 position = options.mercatorPosition.value();

        // Compute minimum allowed mercator Z using the max zoom
        const double minMercatorZ = getCameraToCenterDistance() / Projection::worldSize(std::pow(2.0, getMaxZoom()));
        position[2] = util::max(position[2], minMercatorZ);

        camera.setPosition(position);
    }

    if (options.focusPoint && options.focusPoint.value() != getLatLng()) {
        camera.lookAtPoint(options.focusPoint.value());
    }
